...
struct WoWClientDB
{
	...
	int maxIndex;			// 0x0C
	int minIndex;			// 0x10
	DWORD stringTable;		// 0x14
	DWORD vTable2;			// 0x18
	DWORD FirstRow;			// 0x1C - Address of first row.
	DWORD rowTable;			// 0x20 - Address of array of rows.
};

class ClientDB
{
private:
	WoWClientDB clientDB;
	DWORD address;

public:

	ClientDB(DWORD dbPtr);
	ClientDB(void);
	~ClientDB(void);

	void ClientDB::UpdateHeader();
	
	int DecompressRow(int a1, int a2, int a3);
	int GetLocalizedRow(int recordId, DWORD rowOut);
	
	DWORD GetRow(int record);
	DWORD LookupRow(int a1, int a2, int a3);

	static bool isDBCCompressed();
};

// Idk which patch this is for, but it doesn't seem to work for 3.3.5a 12340.
 /*enum eClientDb
{
